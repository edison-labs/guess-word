...
当前交付是可部署的**工程 MVP**：每日挑战、六分类练习、150 道服务端题目、刷新恢复、三档提示、战绩分享、评分纠偏反馈、本地战绩、DeepSeek 持久评分缓存和 Token/费用统计均已实现。没有真实模型凭据时，可以明确选择测试模式完成全流程；生产环境绝不会静默回退到测试评分。
...
- 新增题目已通过结构、数量和平衡校验，但仍应结合真实玩家的“偏高/偏低”反馈持续校准 DeepSeek 量表。
- 当前有 150 道结构完整题目（六类各 25 道）；适合朋友小范围试玩，公开推广前仍建议扩充到 300 道以上。
- 当前短窗限流是单进程保护；适合朋友小范围试玩，公开推广前必须接入按设备与 IP 协调的网关或分布式限流。
- P0 只接受 NFKC 标准化、去首尾空白后的 1～10 个连续汉字；不做简繁自动转换。
- 每个浏览器只保留本地战绩，未实现登录、跨设备同步或排行榜；每日挑战允许重复开局，暂未限制每天一次。
