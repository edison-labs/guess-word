...
当前交付是可部署的**工程 MVP**：每日挑战、八分类练习、200 道服务端题目、两级渐进提示、同题分享、游客无感身份、短信登录、跨设备用户战绩、每日榜与好友同题榜、DeepSeek 持久评分缓存和 Token/费用统计均已实现。登录不是游玩前置条件；游客登录后，本浏览器由新版账号系统记录的题局会自动合并。没有真实模型凭据时，可以明确选择测试模式完成全流程；生产环境绝不会静默回退到测试评分。
